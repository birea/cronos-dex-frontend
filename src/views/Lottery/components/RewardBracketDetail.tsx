  ...
  return (
    <Flex flexDirection="column">
      {isLoading ? (
        ...
        <Text bold color={isBurn ? 'failure' : 'background'}>
          {getRewardText()}
        </Text>
      )}
      <>
        {isLoading || cakeAmount.isNaN() ? (
          ...
          <Balance color='#3F51B5' fontSize="20px" bold unit=" SALE" value={getBalanceNumber(cakeAmount)} decimals={0} />
        )}
        {isLoading || cakeAmount.isNaN() ? (
          <>
          ...
          <>
            {numberWinners !== '0' && (
              <Text fontSize="12px" color="textSubtle">
                {getFullDisplayBalance(cakeAmount.div(parseInt(numberWinners, 10)), 18, 2)} SALE {t('each')}
              </Text>
            )}
            <Text fontSize="12px" color="textSubtle">
